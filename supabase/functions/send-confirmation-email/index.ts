...
Deno.serve(async (req: Request) => {
  ...
  if (type === 'registration') {
    ...
    to = member.email
    subject = 'Thanks for applying to Uptown Vibez City'
    ...
    if (!member || !member.email) return new Response(JSON.stringify({ error: 'Not found' }), { status: 404, headers: corsHeaders })
    to = member.email
    subject = "You're confirmed! Welcome to Uptown Vibez City"
    ...
    to = profile.email
    subject = 'Your Uptown Vibez City dashboard access is confirmed'
